  ...
  return (
    <div className="w-full max-w-6xl mx-auto space-y-12 py-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 px-4">
        {plans.map((plan) => {
          ...
          return (
            ...
            >
              {plan.name === 'Business' && (
                <div className="absolute top-0 right-6 -translate-y-1/2 bg-indigo-600 text-white text-[9px] font-bold uppercase tracking-widest py-1 px-3 rounded-full shadow-md">
                  Best Value
                </div>
              )}
              ...
              {!isFree && selectedPlanId === plan.id && (
                 <div className="mb-4 space-y-1.5 animate-in slide-in-from-top-2 duration-300">
                    <label className="text-[9px] font-bold uppercase tracking-widest text-indigo-600 ml-1">M-Pesa Number</label>
      ...
      <div className="flex flex-wrap justify-center gap-10 pt-10 grayscale opacity-40">
        ...
        <div className="flex items-center gap-2">
            <Globe size={20} />
            <span className="text-[10px] font-bold uppercase tracking-widest">Global Payments</span>
        </div>
        <div className="flex items-center gap-2">
            <Users size={20} />
            <span className="text-[10px] font-bold uppercase tracking-widest">Team Management</span>
        </div>
      </div>
    </div>
  );
