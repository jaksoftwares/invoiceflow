...
const InvoicePreview = ({
 ...
 <div className="absolute top-1/4 left-1/4 text-[120px] font-bold transform -translate-x-1/2 -translate-y-1/2 rotate-12">
 InvoiceFlow
 </div>
 <div className="absolute bottom-1/4 right-1/4 text-[80px] font-bold transform translate-x-1/2 translate-y-1/2 -rotate-12">
 InvoiceFlow
 </div>
 </div>
 )}
 ...
 <div>
 <h1 className="text-3xl font-bold tracking-tight uppercase mb-1">{businessProfile?.name || 'Professional'}</h1>
 <p className="text-white/70 text-sm font-medium">#{details.invoiceNumber}</p>
 </div>
 </div>
 <div className="text-right">
 <p className="text-4xl font-bold opacity-20 tracking-tight mb-2">{displayType.toUpperCase()}</p>
 ...
 <div>
 <p className="text-[10px] font-bold text-primary uppercase tracking-wider mb-4">Recipient Details</p>
 {client ? (
 <div className="space-y-1">
 <p className="font-bold text-xl text-slate-900 tracking-tight">{client.company_name}</p>
 ...
 <div>
 <p className="text-[10px] font-bold text-slate-400 font-medium mb-1">{effectiveDocumentType === 'quotation' ? 'Valid Until' : effectiveDocumentType === 'receipt' ? 'Date Paid' : 'Due Date'}</p>
 <p className="text-lg font-bold text-slate-900">{formatDate(details.dueDate)}</p>
 </div>
 <div className="bg-primary/5 px-4 py-2 rounded-lg border-l-4 border-primary">
 <p className="text-[10px] font-bold text-primary font-medium mb-1 text-left">Total Balance</p>
 <p className="text-2xl font-bold text-primary tracking-tight">{formatCurrency(total)}</p>
 </div>
 </div>
 </div>
 </div>
 ...
 <th className="py-4 px-4 text-left text-[10px] font-bold text-slate-400 font-medium">Service Description</th>
 <th className="py-4 px-2 text-center text-[10px] font-bold text-slate-400 font-medium w-20">Qty</th>
 <th className="py-4 px-4 text-right text-[10px] font-bold text-slate-400 font-medium w-32">Rate</th>
 <th className="py-4 px-4 text-right text-[10px] font-bold text-slate-400 font-medium w-32">Amount</th>
 </tr>
 </thead>
 ...
 <td className="py-5 px-4 text-sm font-bold text-right text-slate-900">{formatNumber(item.amount)}</td>
 </tr>
 ))}
 </tbody>
 </table>
 ...
 <span className="text-slate-400 font-bold font-medium text-[10px]">Net Value</span>
 <span className="font-bold text-slate-900">{formatCurrency(subtotal)}</span>
 </div>
 {taxRate > 0 && (
 <div className="flex justify-between text-sm items-center">
 <span className="text-slate-400 font-bold font-medium text-[10px]">Sales Tax ({taxRate}%)</span>
 ...
 <span className="font-bold font-medium text-[10px]">Loyalty Discount</span>
 <span className="font-bold">-{formatCurrency(discountAmount)}</span>
 </div>
 )}
 <div className="h-px bg-slate-200 my-4" />
 <div className="flex justify-between items-baseline pt-2">
 <span className="text-sm font-bold text-slate-900 font-medium">{effectiveDocumentType === 'quotation' ? 'Estimated Balance' : effectiveDocumentType === 'receipt' ? 'Remaining Balance' : 'Balance Due'}</span>
 <span className="text-3xl font-bold text-primary tracking-tight">{formatCurrency(total)}</span>
 </div>
 </div>
 </div>
 ...
 <div>
 <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-4">Terms & Instructions</p>
 <p className="text-xs text-slate-500 font-medium leading-relaxed whitespace-pre-wrap">{terms || 'Standard payment terms apply.'}</p>
 </div>
 <div>
 <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-4">Contact Info</p>
 ...
 <p className="text-[9px] font-bold text-primary uppercase tracking-wider">Generated with InvoiceFlow • Premium Billing Solution</p>
 </div>
 </div>
 );
 ...
 {renderLogo('lg')}
 <div>
 <h1 className="text-3xl font-bold tracking-tight text-slate-900 mb-1">{businessProfile?.name}</h1>
 <div className="flex gap-4 text-xs font-bold text-slate-400 font-medium">
 ...
 <div className="text-right">
 <h2 className="text-4xl md:text-5xl font-bold text-primary opacity-10 tracking-tight mb-2 italic">Official</h2>
 <div className="space-y-1 text-right">
 <p className="text-xs font-bold text-slate-400 font-medium">Reference Code</p>
 <p className="text-2xl font-bold tracking-tight">#{details.invoiceNumber}</p>
 </div>
 </div>
 </div>
 ...
 <div>
 <p className="text-[10px] font-bold text-primary uppercase tracking-wider mb-4">Billing To</p>
 <p className="font-bold text-xl tracking-tight mb-2">{client?.company_name}</p>
 ...
 <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-4">Chronology</p>
 <div className="space-y-4">
 <div>
 <p className="text-[10px] font-bold text-slate-400 uppercase mb-1">Creation Date</p>
 <p className="text-sm font-bold">{formatDate(details.issueDate)}</p>
 </div>
 <div>
 ...
 <div className="text-right">
 <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-4">Total Obligation</p>
 <p className="text-3xl font-bold text-primary tracking-tight mb-2">{formatCurrency(total)}</p>
 <p className="text-[10px] font-bold text-slate-400 font-medium">{currency} • {getTermsLabel(details.paymentTerms).toUpperCase()}</p>
 </div>
 </div>

 <h3 className="text-[10px] font-bold text-slate-900 uppercase tracking-wider mb-6 pl-2">Line Items Analysis</h3>
 <div className="overflow-hidden rounded-3xl border border-slate-100 mb-12">
 <table className="w-full">
 <thead>
 <tr className="bg-slate-900 text-white">
 <th className="py-5 px-8 text-left text-[10px] font-bold font-medium">Service Item</th>
 <th className="py-5 px-4 text-center text-[10px] font-bold font-medium w-24">Unit Qty</th>
 <th className="py-5 px-8 text-right text-[10px] font-bold font-medium w-36">Subtotal</th>
 </tr>
 </thead>
 <tbody className="divide-y divide-slate-100">
 {items.map((item, idx) => (
 ...
 <span className="text-base font-bold text-slate-900">{formatNumber(item.amount)}</span>
 </td>
 </tr>
 ))}
 </tbody>
 ...
 <p className="text-[10px] font-bold text-slate-400 font-medium mb-4">Executive Statement</p>
 ...
 <div className="flex justify-between text-[11px] font-bold font-medium opacity-60">
 ...
 {discount > 0 && (
 <div className="flex justify-between text-[11px] font-bold font-medium text-emerald-400">
 <span>Rebate</span>
 <span>-{formatCurrency(discountAmount)}</span>
 </div>
 )}
 ...
 <span className="text-xs font-bold font-medium">Final Total</span>
 <span className="text-2xl font-bold tracking-tight">{formatCurrency(total)}</span>
 </div>
 </div>
 </div>

 <div className="flex justify-between items-center pt-8 border-t border-slate-100 text-[10px] font-bold text-slate-400 uppercase tracking-wider">
 <p>{businessProfile?.website}</p>
 ...
 <div className="text-right">
 <p className="text-[10px] font-bold uppercase tracking-wider opacity-60 mb-2">Statement</p>
 <h1 className="text-5xl font-bold tracking-tight">{displayType.toUpperCase()}.</h1>
 </div>
 </div>
 <div className="flex justify-between items-end">
 <div>
 <p className="text-[10px] font-bold uppercase tracking-wider opacity-60 mb-2">Accountable Entity</p>
 <h2 className="text-2xl font-bold tracking-tight">{businessProfile?.name}</h2>
 ...
 <p className="text-[10px] font-bold font-medium opacity-60 mb-1 capitalize">{displayType} Reference</p>
 <p className="text-xl font-bold tracking-tight">#{details.invoiceNumber}</p>
 </div>
 </div>
 </div>
 </div>
 ...
 <div>
 <p className="text-[10px] font-bold text-slate-400 font-medium mb-6">Dispatch To</p>
 <h3 className="text-2xl font-bold text-slate-900 tracking-tight mb-3">{client?.company_name}</h3>
 ...
 <div>
 <p className="text-[9px] font-bold text-slate-300 uppercase mb-1">Issue Date</p>
 <p className="text-xs font-bold text-slate-800 underline decoration-primary decoration-4 underline-offset-4">{formatDate(details.issueDate)}</p>
 </div>
 <div>
 <p className="text-[9px] font-bold text-slate-300 uppercase mb-1">Terms</p>
 <p className="text-xs font-bold text-slate-800 uppercase tracking-tight">{getTermsLabel(details.paymentTerms)}</p>
 </div>
 </div>
 </div>
 ...
 <p className="text-[10px] font-bold uppercase tracking-wider opacity-40 mb-4">Liability Total</p>
 <h4 className="text-4xl font-bold tracking-tight text-primary mb-4">{formatCurrency(total)}</h4>
 <div className="flex items-center gap-2">
 <div className="w-2 h-2 rounded-full bg-success animate-pulse" />
 <p className="text-[10px] font-bold font-medium text-slate-400">{effectiveDocumentType === 'quotation' ? 'Valid Until ' : effectiveDocumentType === 'receipt' ? 'Paid On ' : 'Due '}{formatDate(details.dueDate)}</p>
 </div>
 </div>
 </div>

 <div className="space-y-6 mb-16">
 <p className="text-[10px] font-bold text-slate-900 uppercase tracking-wider mb-4 pl-4 border-l-4 border-primary">Itemized Inventory</p>
 {items.map((item, idx) => (
 ...
 <p className="font-bold text-slate-900 tracking-tight mb-1">{item.description}</p>
 ...
 <div className="text-right">
 <p className="text-lg font-bold text-slate-900 tracking-tight">{formatNumber(item.amount)}</p>
 </div>
 </div>
 ))}
 </div>
 ...
 <p className="text-[10px] font-bold text-slate-400 font-medium mb-4 italic">Statement Notes</p>
 ...
 <span className="text-[10px] font-bold text-slate-300 font-medium">Base Total</span>
 <span className="text-sm font-bold text-slate-900">{formatCurrency(subtotal)}</span>
 </div>
 {taxRate > 0 && (
 <div className="flex justify-between items-baseline">
 <span className="text-[10px] font-bold text-slate-300 font-medium">Accrued Tax ({taxRate}%)</span>
 <span className="text-sm font-bold text-slate-900">{formatCurrency(taxAmount)}</span>
 </div>
 )}
 {discount > 0 && (
 <div className="flex justify-between items-baseline">
 <span className="text-[10px] font-bold text-emerald-300 font-medium">Settlement Discount</span>
 ...
 <span className="text-sm font-bold text-slate-900 font-medium">{documentType === 'quotation' ? 'Final Estimate' : documentType === 'receipt' ? 'Amount Settled' : 'Final Dues'}</span>
 <span className="text-3xl font-bold text-primary tracking-tight">{formatCurrency(total)}</span>
 </div>
 </div>
 </div>
 ...
 <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Powered by InvoiceFlow Premium</p>
 </div>
 </div>
 </div>
 );
 ...
 <div>
 <h1 className="text-5xl font-bold tracking-tight uppercase leading-none">{businessProfile?.name}</h1>
 <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mt-2">Enterprise Solutions Provider</p>
 </div>
 </div>
 <div className="text-right">
 <div className="space-y-1 mb-8">
 <p className="text-[10px] font-bold text-slate-300 font-medium">Formal Document No.</p>
 <p className="text-4xl font-bold tracking-tight underline underline-offset-8">#{details.invoiceNumber}</p>
 </div>
 <div className="grid grid-cols-2 gap-8 text-right font-bold font-medium">
 <div>
 ...
 <div>
 <p className="text-[11px] font-bold text-primary uppercase tracking-wider mb-8 border-b-2 border-primary/20 pb-4">Recipient Index</p>
 <div className="space-y-2">
 <h3 className="text-3xl font-bold tracking-tight uppercase">{client?.company_name}</h3>
 ...
 <div>
 <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-8 border-b-2 border-slate-100 pb-4">Transaction Overview</p>
 <div className="space-y-6">
 <div className="flex justify-between items-baseline">
 <span className="text-[10px] font-bold text-slate-300 uppercase">Valuation Currency</span>
 <span className="text-sm font-bold">{currency}</span>
 </div>
 <div className="flex justify-between items-baseline">
 <span className="text-[10px] font-bold text-slate-300 uppercase">Payment Lifecycle</span>
 <span className="text-sm font-bold uppercase">{getTermsLabel(details.paymentTerms)}</span>
 </div>
 <div className="flex justify-between items-center p-4 bg-slate-50 rounded-lg">
 <span className="text-[10px] font-bold text-slate-400 uppercase">{documentType === 'quotation' ? 'Principal Estimate' : documentType === 'receipt' ? 'Principal Paid' : 'Principal Balance Due'}</span>
 ...
 <div className="mb-24">
 <p className="text-[11px] font-bold text-slate-900 uppercase tracking-wider mb-10 text-center">Certified Line Item Specification</p>
 <table className="w-full">
 <thead>
 <tr className="border-y-4 border-slate-900">
 <th className="py-6 text-left text-[11px] font-bold font-medium">Detail Item Description</th>
 <th className="py-6 text-right text-[11px] font-bold font-medium w-24">Units</th>
 <th className="py-6 text-right text-[11px] font-bold font-medium w-32">Rate Value</th>
 <th className="py-6 text-right text-[11px] font-bold font-medium w-32">Line Total</th>
 </tr>
 </thead>
 <tbody className="divide-y-2 divide-slate-100">
 {items.map((item, idx) => (
 <tr key={idx} className="group hover:bg-slate-50 transition-colors">
 <td className="py-8 font-bold text-slate-900 text-lg uppercase tracking-tight">{item.description}</td>
 ...
 <td className="py-8 text-right font-bold text-slate-900">{formatNumber(item.amount)}</td>
 </tr>
 ))}
 </tbody>
 </table>
 ...
 <div className="flex justify-between items-center text-[10px] font-bold text-slate-300 font-medium">
 ...
 {discount > 0 && (
 <div className="flex justify-between items-center text-[10px] font-bold text-emerald-300 font-medium">
 ...
 <div className="flex justify-between items-center">
 <span className="text-xs font-bold text-slate-900 uppercase tracking-wider">{documentType === 'quotation' ? 'Final Estimate' : documentType === 'receipt' ? 'Amount Settled' : 'Final Dues Payable'}</span>
 ...
 <div className="max-w-md">
 <p className="text-[11px] font-bold text-slate-900 font-medium mb-4 italic">Enterprise Declarations</p>
 ...
 <div className="text-right">
 <p className="text-[11px] font-bold text-primary font-medium mb-4">Internal Repository Code</p>
 <p className="text-[10px] font-bold text-slate-300 leading-none">AUTH-FLOW-{Date.now().toString().slice(-6)}</p>
 </div>
 </div>
 </div>
 </div>
 ...
 <div className="space-y-2">
 <p className="text-xs font-bold text-primary uppercase tracking-wider capitalize">{displayType}</p>
 <h1 className="text-7xl font-bold tracking-tight text-slate-900">FLOW.</h1>
 </div>
 <div className="text-right">
 <div className="mb-4">
 {renderLogo('md')}
 </div>
 <p className="text-xl font-bold tracking-tight uppercase">{businessProfile?.name}</p>
 <p className="text-[10px] font-bold text-slate-400 font-medium mt-1">#{details.invoiceNumber}</p>
 </div>
 </div>

 <div className="grid grid-cols-12 gap-16 mb-32">
 <div className="col-span-5">
 <p className="text-[10px] font-bold text-slate-300 uppercase tracking-wider mb-10 border-b border-slate-100 pb-4 text-center">Principal Client</p>
 <div className="space-y-4 text-center">
 <h3 className="text-3xl font-bold tracking-tight uppercase text-slate-900">{client?.company_name}</h3>
 <p className="text-sm font-bold text-slate-400 font-medium text-center">{client?.email}</p>
 <div className="pt-4">
 ...
 <div className="grid grid-cols-2 gap-12">
 <div>
 <p className="text-[10px] font-bold text-primary uppercase mb-2">Issue Date</p>
 <p className="text-lg font-bold">{formatDate(details.issueDate)}</p>
 </div>
 <div>
 <p className="text-[10px] font-bold text-primary uppercase mb-2">{documentType === 'quotation' ? 'Valid Until' : documentType === 'receipt' ? 'Date Paid' : 'Due Date'}</p>
 <p className="text-lg font-bold">{formatDate(details.dueDate)}</p>
 </div>
 </div>
 <div className="mt-10 pt-10 border-t border-white/10 flex justify-between items-end">
 <div>
 <p className="text-[10px] font-bold text-slate-400 uppercase mb-2">Settlement Total</p>
 <p className="text-4xl font-bold text-primary tracking-tight">{formatCurrency(total)}</p>
 </div>
 <div className="text-right">
 <p className="text-[9px] font-bold text-slate-500 uppercase">Valuation Index</p>
 <p className="text-xs font-bold">{currency} ({getTermsLabel(details.paymentTerms).toUpperCase()})</p>
 </div>
 </div>
 </div>
 </div>
 ...
 <th className="py-8 text-left text-[10px] font-bold uppercase tracking-wider text-slate-300">Service Description</th>
 <th className="py-8 text-right text-[10px] font-bold uppercase tracking-wider text-slate-300 w-32">Aggregate Amount</th>
 </tr>
 </thead>
 ...
 <td className="py-10">
 <p className="text-2xl font-bold text-slate-900 tracking-tight uppercase group-hover:text-primary transition-colors underline decoration-primary/20">{item.description}</p>
 <p className="text-[10px] font-bold text-slate-300 uppercase mt-2 tracking-widest">{item.quantity} Units x {formatNumber(item.rate)}</p>
 </td>
 <td className="py-10 text-right">
 <span className="text-2xl font-bold text-slate-900 tracking-tight italic">{formatNumber(item.amount)}</span>
 </td>
 </tr>
 ))}
 </tbody>
 ...
 <p className="text-[10px] font-bold text-slate-900 font-medium mb-6 border-l-4 border-primary pl-4">Statement Notes</p>
 ...
 <span className="text-sm font-bold text-slate-900 font-medium border-b-[6px] border-primary/20">Final Balance</span>
 <span className="text-3xl font-bold text-primary tracking-tight group-hover:scale-110 transition-transform">{formatCurrency(total)}</span>
 </div>
 </div>
 </div>

 <div className="mt-40 text-center">
 <p className="text-[10px] font-bold text-slate-200 uppercase tracking-wider mb-4">Official Document</p>
 <p className="text-[9px] font-bold text-primary uppercase tracking-wider">Verified Secure by InvoiceFlow Premium</p>
 </div>
 </div>
 </div>
 );
 ...
 <div className="text-right">
 <h2 className="text-4xl font-bold text-primary/20 tracking-tight uppercase mb-2 capitalize">{displayType}</h2>
 ...
 <h3 className="text-xs font-bold text-slate-400 font-medium mb-3">Client Details</h3>
 {client ? (
 <div className="text-sm">
 ...
 <div>
 <p className="text-[10px] font-bold text-slate-400 font-medium mb-1">Issue Date</p>
 <p className="text-sm font-bold text-slate-900">{formatDate(details.issueDate)}</p>
 </div>
 <div>
 <p className="text-[10px] font-bold text-slate-400 font-medium mb-1">{documentType === 'quotation' ? 'Valid Until' : documentType === 'receipt' ? 'Date Paid' : 'Due Date'}</p>
 <p className="text-sm font-bold text-rose-600">{formatDate(details.dueDate)}</p>
 </div>
 </div>
 <div className="mt-4 pt-4 border-t border-slate-200">
 <p className="text-[10px] font-bold text-slate-400 font-medium mb-1">Payment Terms</p>
 ...
 <th className="text-left py-4 px-2 text-xs font-bold text-slate-900 font-medium">Description</th>
 <th className="text-center py-4 px-2 text-xs font-bold text-slate-900 font-medium w-24">Quantity</th>
 <th className="text-right py-4 px-2 text-xs font-bold text-slate-900 font-medium w-32">Rate</th>
 <th className="text-right py-4 px-2 text-xs font-bold text-slate-900 font-medium w-32">Subtotal</th>
 </tr>
 </thead>
 <tbody>
 {items.map((item, idx) => (
 ...
 <div>
 <h4 className="text-[10px] font-bold text-slate-400 font-medium mb-3">Business Contact</h4>
 ...
 {notes && (
 <div>
 <h4 className="text-[10px] font-bold text-slate-400 font-medium mb-3">Notes & Instructions</h4>
 ...
 <p className="text-[10px] font-bold text-slate-300 uppercase tracking-wider">Official Business Document</p>
 </div>
 </div>
 );
 ...
 <div className="text-right">
 <h2 className="text-xs font-bold uppercase tracking-wider opacity-80 mb-2">{documentType} Number</h2>
 <p className="text-3xl font-bold tracking-tight">#{details.invoiceNumber}</p>
 </div>
 </div>
 ...
 <p className="text-[11px] font-bold text-slate-400 font-medium mb-4">Bill To</p>
 {client ? (
 <div>
 ...
 <div className="col-span-1">
 <p className="text-[11px] font-bold text-slate-400 font-medium mb-4">Dates</p>
 <div className="space-y-4">
 <div>
 ...
 <p className="text-[11px] font-bold text-slate-400 font-medium mb-2">Total Balance</p>
 <p className="text-xl font-bold text-primary tracking-tight">{formatCurrency(total)}</p>
 ...
 <thead>
 <tr className="bg-slate-50">
 <th className="text-left py-4 px-6 text-xs font-bold text-slate-500 font-medium">Service Item</th>
 <th className="text-center py-4 px-2 text-xs font-bold text-slate-500 font-medium w-24">Qty</th>
 <th className="text-right py-4 px-6 text-xs font-bold text-slate-500 font-medium w-32">Amount</th>
 </tr>
 </thead>
 <tbody className="divide-y divide-slate-100">
 {items.map((item, idx) => (
 ...
 <h4 className="text-[11px] font-bold text-slate-400 font-medium mb-3">Additional Notes</h4>
 <p className="text-sm text-slate-600 leading-relaxed italic">{notes}</p>
 </div>
 )}
 </div>

 <div className="bg-slate-900 px-10 py-6 text-white flex justify-between items-center text-[11px] font-bold font-medium opacity-90">
 ...
 <div>
 <h1 className="text-4xl font-bold tracking-wider text-slate-900 uppercase mb-4 capitalize">{displayType}</h1>
 <div className="space-y-1">
 <p className="text-sm font-bold text-slate-400 uppercase font-sans tracking-wider">Reference No.</p>
 <p className="text-lg font-bold text-primary font-sans tracking-widest">{details.invoiceNumber}</p>
 </div>
 </div>
 <div className="text-right">
 {renderLogo('lg')}
 ...
 <h3 className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-4">REMIT TO:</h3>
 ...
 <div className="font-sans">
 <h3 className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-4">BILLING TO:</h3>
 {client ? (
 <div className="text-sm text-slate-700 space-y-2">
 ...
 <thead>
 <tr className="bg-slate-900 text-white">
 <th className="py-4 px-6 text-left text-[10px] font-bold uppercase tracking-wider">Service Description</th>
 <th className="py-4 px-6 text-center text-[10px] font-bold uppercase tracking-wider w-24">QTY</th>
 <th className="py-4 px-6 text-right text-[10px] font-bold uppercase tracking-wider w-32">Rate</th>
 <th className="py-4 px-6 text-right text-[10px] font-bold uppercase tracking-wider w-32">Total</th>
 </tr>
 </thead>
 <tbody className="divide-y-2 divide-slate-100">
 {items.map((item, idx) => (
 ...
 <td className="py-6 px-6 text-sm font-bold text-right text-slate-900">{formatNumber(item.amount)}</td>
 </tr>
 ))}
 </tbody>
 </table>
 ...
 <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-4">Terms & Conditions</h4>
 <p className="text-[11px] text-slate-500 leading-relaxed italic border-l-2 border-slate-200 pl-4">{terms}</p>
 </div>
 )}
 <div className="w-64 space-y-4">
 <div className="flex justify-between text-xs font-bold font-medium">
 ...
 {discount > 0 && (
 <div className="flex justify-between text-xs font-bold font-medium text-emerald-600">
 <span>Discount</span>
 <span>-{formatCurrency(discountAmount)}</span>
 </div>
 )}
 <div className="pt-6 border-t-4 border-slate-900 flex justify-between items-baseline">
 <span className="text-sm font-bold uppercase tracking-wider text-slate-900">{documentType === 'quotation' ? 'Estimated Amount' : documentType === 'receipt' ? 'Paid Amount' : 'Amount Due'}</span>
 <span className="text-xl font-bold text-primary tracking-tight">{formatCurrency(total)}</span>
 </div>
 </div>
 </div>

 <div className="text-center pt-8 border-t border-slate-100 font-sans">
 <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2">{businessProfile?.name || 'Executive Pro'}</p>
 <p className="text-[9px] text-slate-300 font-medium italic">{businessProfile?.website || ''}</p>
 </div>
 </div>
 </div>
 );
 ...
 <div className="flex justify-between items-start mb-24">
 <div>
 <h1 className="text-4xl md:text-5xl font-bold tracking-tight text-slate-900 mb-2 capitalize">{displayType}.</h1>
 ...
 <p className="font-bold text-xl font-medium">{businessProfile?.name}</p>
 </div>
 </div>

 <div className="grid grid-cols-4 gap-12 mb-24">
 <div className="col-span-1">
 <h3 className="text-[10px] font-bold text-slate-300 font-medium mb-6 underline decoration-primary decoration-4 underline-offset-8">Client</h3>
 {client ? (
 ...
 <div className="col-span-1">
 <h3 className="text-[10px] font-bold text-slate-300 font-medium mb-6 underline decoration-primary decoration-4 underline-offset-8">Issued</h3>
 <p className="text-sm font-bold text-slate-900 uppercase tracking-wider">{formatDate(details.issueDate)}</p>
 </div>
 <div className="col-span-1">
 <h3 className="text-[10px] font-bold text-slate-300 font-medium mb-6 underline decoration-primary decoration-4 underline-offset-8">Deadline</h3>
 <p className="text-sm font-bold text-slate-900 uppercase tracking-wider">{formatDate(details.dueDate)}</p>
 </div>
 <div className="col-span-1 text-right">
 <h3 className="text-[10px] font-bold text-slate-300 font-medium mb-6 underline decoration-primary decoration-4 underline-offset-8">Total</h3>
 <p className="text-xl font-bold text-slate-900 tracking-tight">{formatCurrency(total)}</p>
 </div>
 </div>

 <div className="mb-24">
 <div className="grid grid-cols-12 gap-6 border-b border-slate-100 pb-4 mb-4">
 <div className="col-span-8 text-[10px] font-bold text-slate-400 font-medium">Detail Description</div>
 <div className="col-span-2 text-center text-[10px] font-bold text-slate-400 font-medium">Amt</div>
 <div className="col-span-2 text-right text-[10px] font-bold text-slate-400 font-medium">Sub</div>
 </div>
 ...
 <p className="text-[10px] text-slate-300 font-bold font-medium mt-2">{item.quantity} Unit(s) at {formatNumber(item.rate)} / unit</p>
 </div>
 <div className="col-span-2 text-center text-sm font-medium text-slate-400">
 {formatNumber(item.amount)}
 </div>
 <div className="col-span-2 text-right text-base font-bold text-slate-900">
 {formatNumber(item.amount)}
 </div>
 </div>
 ))}
 ...
 {notes && (
 <div className="max-w-md">
 <h4 className="text-[10px] font-bold text-slate-900 font-medium mb-4">Statement Notations.</h4>
 ...
 <div className="flex justify-between items-baseline">
 <span className="text-xs font-bold text-slate-300 font-medium">Net Total.</span>
 <span className="text-sm font-bold text-slate-900">{formatCurrency(subtotal)}</span>
 </div>
 {taxRate > 0 && (
 <div className="flex justify-between items-baseline">
 <span className="text-xs font-bold text-slate-300 font-medium">Calculated Tax.</span>
 <span className="text-sm font-bold text-slate-900">+{formatCurrency(taxAmount)}</span>
 </div>
 )}
 {discount > 0 && (
 <div className="flex justify-between items-baseline text-emerald-500">
 <span className="text-xs font-bold font-medium opacity-60">Applied Discount.</span>
 ...
 <span className="text-sm font-bold uppercase tracking-wider text-slate-900 font-sans">{documentType === 'quotation' ? 'Final Estimate.' : documentType === 'receipt' ? 'Amount Settled.' : 'Final Balance Due.'}</span>
 <span className="text-xl font-bold text-primary tracking-tight">{formatCurrency(total)}</span>
 </div>
 </div>
 </div>
 
 <div className="mt-32 flex justify-between items-center text-[10px] font-bold uppercase tracking-wider text-slate-200">
 ...
 <div className="space-y-1">
 <h1 className="text-4xl font-bold tracking-tight text-slate-900 uppercase">{businessProfile?.name || 'Bold Business'}</h1>
 <p className="text-primary font-bold font-medium text-sm">{businessProfile?.website || 'Elite Solutions'}</p>
 </div>
 </div>
 <div className="text-right">
 <h2 className="text-4xl md:text-5xl font-bold text-slate-100 absolute right-12 top-10 pointer-events-none uppercase transform scale-y-125 origin-right capitalize">{displayType}</h2>
 <div className="relative z-10 space-y-1">
 <p className="text-xs font-bold text-slate-400 font-medium">Document Registry</p>
 <p className="text-2xl font-bold text-slate-900 tracking-tight">#{details.invoiceNumber}</p>
 </div>
 </div>
 </div>
 ...
 <div>
 <p className="text-xs font-bold text-primary uppercase tracking-wider mb-4">Client Representative</p>
 {client ? (
 <div className="space-y-3">
 <h3 className="text-2xl font-bold text-slate-900 leading-tight">{client.company_name}</h3>
 ...
 <p className="text-[10px] font-bold text-white/50 font-medium mb-1">{documentType === 'quotation' ? 'Valid Until' : documentType === 'receipt' ? 'Date Paid' : 'Due Date'}</p>
 <p className="text-sm font-bold">{formatDate(details.dueDate)}</p>
 </div>
 </div>
 </div>
 ...
 <tr>
 <th className="py-5 px-8 text-left text-[11px] font-bold font-medium">Billable Item</th>
 <th className="py-5 px-4 text-center text-[11px] font-bold font-medium w-24">Unit</th>
 <th className="py-5 px-8 text-right text-[11px] font-bold font-medium w-40">Amount</th>
 </tr>
 </thead>
 <tbody className="divide-y-2 divide-slate-100">
 {items.map((item, idx) => (
 ...
 <td className="py-6 px-8">
 <p className="text-lg font-bold text-slate-900 leading-none">{item.description}</p>
 ...
 <td className="py-6 px-8 text-right font-bold text-xl text-slate-900 tracking-tight">
 {formatNumber(item.amount)}
 </td>
 </tr>
 ))}
 ...
 <h4 className="text-[10px] font-bold text-slate-400 font-medium mb-2">Internal Notes</h4>
 <p className="text-sm text-slate-700 font-medium leading-relaxed">{notes}</p>
 </div>
 )}
 </div>
 <div className="w-80 space-y-4">
 <div className="flex justify-between text-xs font-bold font-medium px-4">
 ...
 {discount > 0 && (
 <div className="flex justify-between text-xs font-bold font-medium px-4 text-emerald-500">
 ...
 <div className="flex justify-between items-center">
 <div className="space-y-1">
 <p className="text-[10px] font-bold text-slate-400 font-medium">Total Valuation</p>
 <p className="text-base font-bold uppercase">{currency} Total</p>
 </div>
 <p className="text-xl font-bold tracking-tight">{formatCurrency(total)}</p>
 </div>
 </div>
 </div>
 </div>

 <div className="mt-20 flex justify-between items-center text-[10px] font-bold uppercase tracking-wider text-slate-400">
 <div className="flex items-center gap-4">
 ...
 <div className="text-right">
 <p className="text-amber-500 text-xs font-semibold font-medium capitalize">{displayType}</p>
 ...
 <p className="text-xs font-sans text-slate-400 font-medium mt-2">
 {businessProfile?.city} • {businessProfile?.country}
 </p>
 </div>
 </div>
 <div className="text-right">
 <p className="text-xs font-sans font-semibold text-slate-400 font-medium capitalize">{displayType} No.</p>
 ...
 <div>
 <p className="text-xs font-sans font-semibold text-slate-400 font-medium mb-3">Prepared For</p>
 {client ? (
 <div>
 ...
 {terms && (
 <div className="mt-8 pt-6 border-t border-slate-200">
 <p className="text-xs font-sans font-semibold text-slate-400 font-medium mb-2">Terms & Conditions</p>
 ...
 <p className="text-xs font-sans font-semibold text-slate-300 uppercase tracking-wider">
 ...
 <div className="flex justify-between items-start">
 <div>
 <h1 className="text-3xl font-bold tracking-tight">{displayType.toUpperCase()}</h1>
